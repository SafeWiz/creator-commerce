...
```
getDownloadableProductFile(productId, userId) → { fileKey, fileName } | null
```
...
- `new Response(null, { status: 404 })` rather than `notFound()`. `notFound()` is
  specified in terms of rendering a `not-found` segment — the wrong shape for a
  handler whose success case is a redirect.
...
| Case | Response |
| --- | --- |
| Non-numeric or non-positive `productId` | 404, no query run |
| Signed out | 302 to `/login?next=%2Fdownloads` |
| Product missing, not owned, not purchased, or fileless | 404 |
...
3. As the seller, download from `/products/[id]` → file arrives, as an
   attachment.
4. As a signed-in non-buyer, `GET /downloads/[id]` → 404.
